...
const Icon: React.FC<IconProps> = ({ name, className = '', style, size, color = 'black' }) => {
  const sizeClass = size ? `fa-${size}` : '';
  const colorClass = color !== 'black' ? `notes-color-${color}` : '';
  
  const combinedClassName = getClasses(`fa fa-${name}`, sizeClass, colorClass, className);
  
  const iconStyle = color !== 'black' ? { color: `var(--notes-${color})`, ...style } : style;

  return <i className={combinedClassName} style={iconStyle} aria-hidden="true" />;
};
