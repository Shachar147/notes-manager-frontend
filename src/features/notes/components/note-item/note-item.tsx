      ...
      <ListItemText
        primary={
          <Tooltip title={note.title || 'New Note'} placement="top" arrow>
            <span
              className={getClasses('notes-headline-6', styles.newNoteTitle)}
            >
              {note.title || 'New Note'}
            </span>
          </Tooltip>
        ...
        secondary={
          <>
            <span className="notes-secondary">{formattedDate}</span>
            <span className={getClasses('notes-body', styles.noteContent)}>
              —{' '}
              {stripHtml(
